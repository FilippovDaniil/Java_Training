...
## Путь, который мы прошли (части 1–4)
...
Один упакованный сервис готов. Но в продукте сервисов много, и синхронные HTTP-вызовы между ними быстро упираются в предел: получатель обязан быть доступен сейчас, пики нагрузки передаются вниз без сглаживания, а каждый новый подписчик требует правок в отправителе.

В [модуле 119](../m119_kafka_intro/theory.md) начинается **часть 5 — асинхронное взаимодействие через Apache Kafka**: брокер, топики/партиции/офсеты, запуск в Compose рядом с нашим стеком и CLI. Дальше — producer и consumer на Spring, контракты событий, надёжность (retry/DLQ/идемпотентность), событийные паттерны (outbox, CQRS, saga), тестирование и эксплуатация. К концу части Task Tracker получит сервис-подписчик и станет распределённой системой.
