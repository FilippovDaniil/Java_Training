...
Сервлет не запускается сам — им управляет **сервлет-контейнер** (см. [модуль 40](../m40_servlet_containers/theory.md)).
...
> ⚠️ Один экземпляр сервлета обслуживает **много** запросов в разных потоках. Поэтому поля сервлета — общие; не храните в них данные конкретного запроса (проблема потокобезопасности, см. [модуль 25](../m25_multithreading_concurrency/theory.md)).
...
В [модуле 40](../m40_servlet_containers/theory.md) — контейнеры сервлетов и развёртывание.
