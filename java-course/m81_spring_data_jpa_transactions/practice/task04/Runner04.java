...
class Runner04 implements CommandLineRunner {
    ...
    @Autowired
    Runner04(ProductRepository04 repo, FailService04 service) {
        this.repo = repo;
    ...
    public void run(String... args) {
        try {
            service.saveAndFailUnchecked("A");
        } catch (Exception e) {
            System.out.println("unchecked: " + e.getMessage());
        }

        try {
            service.saveAndFailChecked("B");
        }   catch (Exception e) {
        ...
        System.out.println("Осталось записей: " + repo.count());
    }
}
