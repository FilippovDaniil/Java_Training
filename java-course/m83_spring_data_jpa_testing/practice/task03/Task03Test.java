...
class Task03Test {
    ...
    @Autowired
    public Task03Test(ProductRepository03 repo, TestEntityManager em) {
        this.repo = repo;
        this.em = em;
    }
    ...
    void seed() {
        ...
        em.persist(new Product03("Хлеб", "Еда", 50));
    ...
    void filters_by_price() {
        ...
        List<Product03> dear = repo.findByPriceGreaterThan(100);
        assertThat(dear).allMatch(p -> p.getPrice() > 100);
    }
    ...
    void sorted_by_price() {
        ...
        List<Product03> food = repo.findByCategoryOrderByPriceAsc("Еда");
        assertThat(food).isSortedAccordingTo(Comparator.comparingInt(Product03::getPrice));
    }

    @Test
    void counts_by_category() {
        // TODO: assertThat(repo.countByCategory("Техника")).isEqualTo(2);
        assertThat(repo.countByCategory("Техника")).isEqualTo(2);
    }
}
