...
ORM удобен, но «прозрачность» дорого стоит, если не понимать стоимость операций. N+1 мы закрыли ([модуль 86](../module-86-hibernate-deep-dive-fetching/theory.md)). Здесь — массовые вставки/обновления, экономия памяти и кэширование.
...
> ⚠️ `GenerationType.IDENTITY` **отключает** батчинг вставок: Hibernate обязан получить сгенерированный id после каждого INSERT построчно. Для bulk-insert берите `SEQUENCE` ([модуль 88](../module-88-hibernate-deep-dive-modeling/theory.md)).
...
> Bulk идёт **мимо** persistence context (см. [модуль 87](../module-87-hibernate-deep-dive-querying/theory.md)) — после него `em.clear()`/`refresh()`. Зато на порядки быстрее для массовых изменений.
...
Или транзакция `@Transactional(readOnly = true)` ([модуль 81](../module-81-spring-data-jpa-transactions/theory.md)). Меньше памяти, быстрее (нет сравнения при flush).
...
- [модуль 86](../module-86-hibernate-deep-dive-fetching/theory.md) — устранение N+1 (главный перформанс-фактор).

## Что дальше

В [модуле 92](../module-92-hibernate-deep-dive-diagnostics/theory.md) — диагностика: логирование SQL, `Statistics` API, тестирование числа запросов (ловля N+1 в тестах), каталог анти-паттернов и финальный аудит. Это завершает курс по Hibernate.
