...
`java.nio.file` (New I/O) — современный API для работы с файлами. Он удобнее и компактнее классического `java.io` из [модуля 18](../module-18-io-streams/theory.md).
...
> `Files.list` возвращает Stream (см. [модуль 24](../module-24-stream-api-lambda/theory.md)) — его нужно закрывать (try-with-resources).
...
В [модуле 20](../module-20-date-time/theory.md) — работа с датой и временем.
