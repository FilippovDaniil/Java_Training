...
> Предварительные знания: [модуль 60](../module-60-spring-core-beans/theory.md) — основы IoC-контейнера, `@Component`, `@Autowired`, `AnnotationConfigApplicationContext`.
...
В [модуле 62](../module-62-spring-core-configuration/theory.md) — конфигурация через `@Configuration` / `@Bean`, профили (`@Profile`), внешние свойства (`@Value`, `@ConfigurationProperties`).
