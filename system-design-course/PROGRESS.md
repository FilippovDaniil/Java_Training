...
**На текущий момент:** 🟡 **В работе — готовы Батчи 1–6 (Части 1–4, модули 01–20): 20/49.** Сгенерировано 240 .md (20×[theory + 10 задач + questions]), скан на кракозябры/NUL/CJK — чисто. **Части 1–4 завершены (Часть 4 — 3/3).** Дальше Батч 7 (модули 21–23: CAP/PACELC, координация/консенсус, надёжность операций — Часть 5 «Распределённые системы»). Детальный план каждого модуля (что обязательно в `theory.md` + темы 10 задач) дописывается в этот файл в начале соответствующего батча, перед генерацией.
