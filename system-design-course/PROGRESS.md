...
**На текущий момент:** 🟡 **В работе — готовы Батчи 1–5 (Части 1–2 + Часть 3 целиком, модули 01–17): 17/49.** Сгенерировано 204 .md (17×[theory + 10 задач + questions]), скан на кракозябры/NUL/CJK — чисто. **Часть 3 завершена (10/10).** Дальше Батч 6 (модули 18–20: OLTP/OLAP/DWH, Lake/Lakehouse/Mesh, конвейеры данных — Часть 4). Детальный план каждого модуля (что обязательно в `theory.md` + темы 10 задач) дописывается в этот файл в начале соответствующего батча, перед генерацией.
