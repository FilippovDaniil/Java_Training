...
**На текущий момент:** 🟡 **В работе — готовы Батчи 1–7 (Части 1–5, модули 01–23): 23/49.** Сгенерировано 276 .md (23×[theory + 10 задач + questions]), скан на кракозябры/NUL/CJK — чисто. **Части 1–5 завершены (Часть 5 — 3/3).** Дальше Батч 8 (модули 24–26: масштабируемость/отказоустойчивость, multi-region/DR, узкие места — Часть 6 «Надёжность/производительность/масштаб», под-батч a). Детальный план каждого модуля (что обязательно в `theory.md` + темы 10 задач) дописывается в этот файл в начале соответствующего батча, перед генерацией.
